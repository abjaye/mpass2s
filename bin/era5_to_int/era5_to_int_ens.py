...
# Changed by Abby Jaye 06/2025 to be able to read ERA5-ensemble data
...
def find_era5_file(var, validtime, localpaths=None):
    ...
    for p in file_paths:
        ...
        if var.isInvariant:
            ...
            # directories.
            #base_bath = p
            base_path = "/glade/campaign/collections/rda/data/d633006/e5.oper.invariant/"
        else:
            base_path = p
            #base_path = "/glade/campaign/collections/rda/data/d633006/e5.oper.invariant/"
...
def find_time_index(ncfilename, validtime):
    ...
    with Dataset(ncfilename) as f:
        # Needed to add these four lines to make the data from CDS work
        epoch_time = f.variables['valid_time'][0]
        newdate = datetime.datetime.fromtimestamp(f.variables['valid_time'][0], datetime.UTC)
        yyyye = newdate.year; mme = newdate.month; dde = newdate.day; hhe = newdate.hour
        utc_date = yyyye * 1000000 + mme * 10000 + dde * 100 + hhe
...
def handle_datetime_args(args):
    ...
    return startDate, endDate, intvH, yyyy, mm, dd
...
if __name__ == '__main__':
    ...
    parser.add_argument('-po', '--po', help='where to write the output file')
    ...
    parser.add_argument('-e', '--ens', help='ensemble number integer 0-9')
    args = parser.parse_args()

    try:
        startDate, endDate, intvH, yyyy, mm, dd = handle_datetime_args(args)
    ...
    int_vars = []
    #if args.isobaric:
    int_vars.append(MetVar('GEOPT', 'z', pl_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SPECHUMD', 'q', pl_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('TT', 't', pl_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('UU', 'u', pl_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('VV', 'v', pl_filename, begin_daily, end_daily, LatLon))
    #else:
    ...
    #int_vars.append(MetVar('LANDSEA', 'lsm', sfc_filename, begin_daily, end_daily, LatLon, isInvariant=True))
    int_vars.append(MetVar('LANDSEA', 'lsm', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SST', 'sst', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SKINTEMP', 'skt', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SM000007', 'swvl1', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SM007028', 'swvl2', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SM028100', 'swvl3', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SM100289', 'swvl4', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('ST000007', 'stl1', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('ST007028', 'stl2', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('ST028100', 'stl3', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('ST100289', 'stl4', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SEAICE', 'siconc', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('TT', 't2m', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('DEWPT', 'd2m', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('UU', 'u10', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('VV', 'v10', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SNOW_DEN', 'rsn', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('SNOW_EC', 'sd', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('PMSL', 'msl', sfc_filename, begin_daily, end_daily, LatLon))
    int_vars.append(MetVar('PSFC', 'sp', sfc_filename, begin_daily, end_daily, LatLon))
    #int_vars.append(MetVar('SOILGEO', 'z', sfc_filename, begin_daily, end_daily, LatLon, isInvariant=True))
    int_vars.append(MetVar('SOILGEO', 'z', sfc_filename, begin_daily, end_daily, LatLon))
    ...
    while currDate <= endDate:
        ...
        ens = args.ens
        intfile = WPSUtils.IntermediateFile(pathout+'/ERA5', initdate)
        ...
        for v in int_vars:
            ...
            with Dataset(e5filename) as f:
                ...
                yyyye = newdate.year; mme = newdate.month; dde = newdate.day; hhe = newdate.hour
                utc_date = yyyye * 1000000 + mme * 10000 + dde * 100 + hhe
                hdate = intdate_to_string(utc_date)
                ...
                # I do think this is where the actualy data is read in and 
                # can select ensemble member. Or it could be in the if statements
                # below where field_arr becomes 'slab'.
                #field_arr = f.variables[v.ERA5name][idx,:]
                #ens = 0
                field_arr = f.variables[v.ERA5name][ens,idx,:]
                ...
                if field_arr.ndim == 2:
                    ...
                    for k in range(f.dimensions['pressure_level'].size)[::-1]:
                        slab = field_arr[k,:,:]
                        ...
                        if args.isobaric:
                            xlvl = f.variables['pressure_level'][k] * 100.0    # Convert hPa to Pa
                        else:
                            xlvl = float(f.variables['pressure_level'][k])     # Level index
